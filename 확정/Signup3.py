...
class ReservationApp:
    ...
    def create_widgets(self):
        ...
        tk.Radiobutton(self.root, text=f'2시간 : {database.menu(database.event_Value)["2h"]}원',variable=self.myVar1, value=0).grid(row=3, column=1, padx=10, pady=10)
        tk.Radiobutton(self.root, text=f'4시간 : {database.menu(database.event_Value)["4h"]}원',variable=self.myVar1, value=1).grid(row=4, column=1, padx=10, pady=10)
        tk.Radiobutton(self.root, text=f'6시간 : {database.menu(database.event_Value)["6h"]}원',variable=self.myVar1, value=2).grid(row=5, column=1, padx=10, pady=10)
        tk.Radiobutton(self.root, text=f'12시간 : {database.menu(database.event_Value)["12h"]}원',variable=self.myVar1, value=3).grid(row=6, column=1, padx=10, pady=10)
        tk.Radiobutton(self.root, text=f'1일 : {database.menu(database.event_Value)["1 day"]}원',variable=self.myVar1, value=4).grid(row=7, column=1, padx=10, pady=10)
        tk.Radiobutton(self.root, text=f'7일 : {database.menu(database.event_Value)["7 day"]}원',variable=self.myVar1, value=5).grid(row=8, column=1, padx=10, pady=10)
        tk.Radiobutton(self.root, text=f'30일 : {database.menu(database.event_Value)["30 day"]}원',variable=self.myVar1, value=6).grid(row=9, column=1, padx=10, pady=10)
    ...
    def myTime(self):
        ...
        elif self.myVar1.get() == 1:
            return timedelta(hours=4)
        elif self.myVar1.get() == 2:
            return timedelta(hours=6)
        elif self.myVar1.get() == 3:
            return timedelta(hours=12)
        elif self.myVar1.get() == 4:
            return timedelta(days=1)
        elif self.myVar1.get() == 5:
    ...
    def payment(self, master):
        ...
        def set_price():
            self.set_price()  # set_price 함수 호출
            label_var.set(f"결제 금액: {self.payment_amount}")
