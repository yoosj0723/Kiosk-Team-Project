...
def main():
    ...
    reservation_app_instance = None  # ReservationApp 인스턴스를 저장할 변수

    def button_clicked1():
        nonlocal reservation_app_instance
        # 인스턴스가 이미 존재하면 새로 생성하지 않고 기존 인스턴스를 재사용
        if not reservation_app_instance:
            reservation_app_instance = ReservationApp()
